...
result = paral %dopar% {
  ...
  for(rep_i in 1:rep_num){
    ...
    for(t in (K+1):T){
      ...
      mod = picasso(Xmat, Ymat, method="scad", lambda=lambda1, intercept=FALSE, standardize = FALSE)
...
stopCluster(cl)
save(params_list, result, file="scad_ucb_vs_cv.RData")
...
scad_ucb_vs_cv = result
params_list[which.min(scad_ucb_vs_cv[, T]), ]
min(scad_ucb_vs_cv[, T])
