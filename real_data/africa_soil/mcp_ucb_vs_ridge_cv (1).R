...
print('Experiment for mcp ucb with variable selection and ridge regression')
      ...
      if (sd(Ymat) == 0){
        ...
        if ((t - 1 - init) == 0){
          ...
          for (i in 1:length(lambda0_list)){
            ...
            for (k in 1:nfolds){
              cv_fit = ncvreg(Xmat[fold!=k, ], Ymat[fold!=k], standardize=F, penalty="MCP", lambda=lambda, alpha=alpha, intercept=T)
        ...
        if ((t - 1 - init) %in% cv_times[-1]){
          ...
          for (i in 1:nrow(cv_params_list)){
            ...
            for (k in 1:nfolds){
              cv_fit = ncvreg(Xmat[fold!=k, ], Ymat[fold!=k], standardize=F, penalty="MCP", lambda=lambda, alpha=cv_params_list[i, 2], intercept=T)
              yhat = predict(cv_fit, Xmat[fold==k, ], type="response")
              loss = (Ymat[fold==k] - yhat) ^ 2
              E[fold==k, i] = loss
            }
        ...
        if (length(active_set) > 1){
          ...
          if ((t - 1 - init) %in% cv_times){
            cv.mod2 = cv.glmnet(Xmat[, active_set], Ymat, alpha=0, standardize=FALSE, nfold=nfolds)
...
stopCluster(cl)
save(params_list, result, file="mcp_ucb_vs_ridge_cv.RData")
...
mcp_ucb_vs_ridge_cv_mat = mcp_ucb_vs_ridge_cv[, Horizon]
mcp_ucb_vs_ridge_cv_mat
